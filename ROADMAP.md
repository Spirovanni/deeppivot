...
- [x] Frontend: Job detail page at /dashboard/jobs/[jobId] (deeppivot-325) — Server-rendered job detail with full description, company sidebar (logo/industry/location/website/size), salary/type/level/remote metadata, match score with "Why you match" tooltip, Apply Now button with ApplyModal, already-applied state with link to My Applications, sign-in gate for unauthenticated users, position-closed indicator, loading skeleton; JobDetailApply client component
- [x] Feature: Save marketplace job to Practice Library (deeppivot-326) — SaveToLibraryButton on job detail page POSTs to existing `/api/job-descriptions` endpoint, mapping marketplace job title/company/description into the user's JD library for context-aware interview practice; auto-triggers LLM extraction + embedding; success state with link to `/dashboard/practice/job-descriptions`
